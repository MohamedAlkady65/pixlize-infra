...
declare -A codepipeline_app_front
codepipeline_app_front[name]="$prefix-app-front-codepipeline"

declare -A codepipeline_app_front_role
codepipeline_app_front_role[name]="${codepipeline_app_front[name]}-role"

codepipeline_app_front_role[policy_name]="${codepipeline_app_front_role[name]}-policy"

codepipeline_app_front_role[policy_document]=$(cat <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
    ...
    },
    {
      ...
      "Resource": "${codebuild_app_front[arn]}"
    },
    {
      "Sid": "CodeDeploy",
      "Effect": "Allow",
...
codepipeline_app_front_role[assume_documnet]=$(cat <<EOF
{
    "Version": "2012-10-17",
    "Statement": [
        {
...
create_role "${codepipeline_app_front_role[name]}" "${codepipeline_app_front_role[assume_documnet]}"
codepipeline_app_front_role[id]="$rt1"
codepipeline_app_front_role[arn]="$rt2"

print_sperator

put_policy_to_role "${codepipeline_app_front_role[name]}" "${codepipeline_app_front_role[policy_name]}" "${codepipeline_app_front_role[policy_document]}" 
